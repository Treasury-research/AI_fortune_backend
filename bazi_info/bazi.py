...
@capture_print
def baziAnalysis(options,mingyun_analysis,chushen_analysis):
    def gan_zhi_he(zhu):
        gan, zhi = zhu
    ...
    # print(options.year, options.month, options.day, options.time, options.month[1], options.month[0])
    ...
    if not options.b:
        ...
        #yun_day = "{}年{}月{}日"
        print("农历:", end='')
        print("\t{}年{}{}月{}日 穿=害 ".format(day.getLunarYear(), Lleap, day.getLunarMonth(), day.getLunarDay(), yun.getStartSolar().toFullString()))
    print("-"*120)
    print("墓库：", str(kus).replace("'",""), end=' ')
    ...
    # print(' '.join(list(gans)), ' '*5, ' '.join(list(gan_shens)), out,)
    ...
    # print(' '.join(list(zhis)), ' '*5, ' '.join(list(zhi_shens)) ,  "生：寅申巳亥 败：子午卯酉　库：辰戌丑未", ' '*2,  out)
